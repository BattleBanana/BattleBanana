...
async def update_server_count():
    await _post_shard_count_bod(BOTS_ON_DISCORD, config["botsOnDiscordKey"])
    await _post_shard_count_topgg(TOP_GG, config["topGGKey"])
    await _post_shard_count_discord_labs(DISCORD_LABS, config["discordLabsKey"])
...
async def _post_shard_count_rovel(site, key):
    ...
    await _post_server_count(site, key, payload)
...
async def _post_shard_count_discord_labs(site, key):
    payload = {"server_count": util.get_server_count(), "shard_count": util.get_shard_count()}

    await _post_server_count(site, key, payload)


async def _post_shard_count_topgg(site, key):
    payload = {"server_count": util.get_server_count(), "shard_count": util.get_shard_count()}

    await _post_server_count(site, key, payload)


async def _post_server_count(site, key, payload):
