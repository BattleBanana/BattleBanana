...
async def acceptallquests(ctx, **details): 
    ...
    if sum([quest.money for quest in player.quests]) // 2 > player.money:
    ...
    battle_embed.add_field(name="Results", value=("Money: `¤"+str(totalCash)+"`\nAdded EXP: `"+ str(round(totalXp))+"`\n"+ e.ATK+" "+str(round(totalAttack,2))+"\n"+ e.ACCY+" "+str(round(totalAccuracy,2))+"\n" + e.STRG +" "+str(round(totalStrength,2))))
