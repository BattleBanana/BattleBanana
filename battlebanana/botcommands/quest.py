...
@commands.command(permission=Permission.MOD, args_pattern="S?P?C?", hidden=True)
...
async def createquest(ctx, name, attack, strg, accy, hp,
    ...
    if len(quests.get_server_quest_list(ctx.server)) >= gconf.THING_AMOUNT_CAP:
        raise util.BattleBananaException(ctx.server, "Whoa, you've reached the limit of %d quests!"
