...
import battlebanana.permissions
...
async def permissions(ctx, **_):
    ...
    permissions_report = ""
    for permission in battlebanana.permissions.permissions:
        permissions_report += ("``" + permission.value[1] + "`` → "
                               + (":white_check_mark:" if battlebanana.permissions.has_permission(ctx.author,
                                                                                             permission)
                                  else ":no_entry:") + "\n")
...
@commands.command(permission=Permission.MOD, args_pattern="SSSSIP?")
async def uploadbg(ctx, icon, name, description, url, price, submitter=None, **details):
    ...
    if not (util.char_is_emoji(icon) or util.is_server_emoji(ctx.server, icon)):
        raise util.BattleBananaException(ctx.channel, "Icon must be emoji available on this server!")
...
@commands.command(permission=Permission.MOD, args_pattern="S")
...
@commands.command(permission=Permission.ADMIN, args_pattern="S")
...
@commands.command(permission=Permission.ADMIN, args_pattern="CC?B?", hidden=True)
async def generatecode(ctx, value, count=1, show=True, **details):
    ...
    newcodes = ""
    with open("battlebanana/game/configs/codes.json", "r+") as code_file:
...
@commands.command(permission=Permission.ADMIN, args_pattern="C?")
...
@commands.command(permission=Permission.OWNER, args_pattern="PS")
...
@commands.command(permission=Permission.ADMIN, args_pattern="PC")
async def setpermlevel(ctx, player, level, **_):
    ...
    permission_list = battlebanana.permissions.permissions
    if permission_index < len(permission_list):
            permission = permission_list[permission_index]
            battlebanana.permissions.give_permission(member, permission)
...
@commands.command(permission=Permission.ADMIN, args_pattern="P", aliases=["giveban"])
async def ban(ctx, player, **_):
    if (player.id == "115269304705875969" or (player.id == "261799488719552513")):
        raise util.BattleBananaException(ctx.channel, "You cannot ban DeveloperAnonymous or Firescoutt")
    battlebanana.permissions.give_permission(player.to_member(), Permission.BANNED)
...
@commands.command(permission=Permission.ADMIN, args_pattern="P", aliases=["pardon"])
async def unban(ctx, player, **_):
    member = player.to_member()
    if not battlebanana.permissions.has_special_permission(member, Permission.BANNED):
        await util.say(ctx.channel, "**%s** is not banned..." % player.name_clean)
        return
    battlebanana.permissions.give_permission(member, Permission.PLAYER)
...
@commands.command(permission=Permission.ADMIN, args_pattern=None, hidden=True)
async def bans(ctx, **_):
    bans_embed = discord.Embed(title="Ban list", type="rich", color=gconf.DUE_COLOUR)
    stringe = ""
    for k, v in battlebanana.permissions.special_permissions.items():
        if v == "banned":
            stringe += "<@%s> (%s)\n" % (k,k)
...
@commands.command(permission=Permission.ADMIN, args_pattern="P")
...
@commands.command(permission=Permission.ADMIN, args_pattern=None)
...
@commands.command(permission=Permission.ADMIN, args_pattern="IP*")
...
@commands.command(permission=Permission.ADMIN, args_pattern="PI")
...
@commands.command(permission=Permission.ADMIN, args_pattern="PS")
...
@commands.command(permission=Permission.ADMIN, args_pattern="PR")
...
@commands.command(permission=Permission.MOD, args_pattern=None)
