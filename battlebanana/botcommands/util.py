...
async def botinfo(ctx, **_):
    ...
    info_embed.add_field(name="Created by", value="[MacDue#4453](https://battlebanana.tech/)")
...
async def shutupdue(ctx, *args, **details):
    ...
    using ``[CMD_KEY]shutupdue all`` will make BattleBanana ignore all commands
    from non-admins.
...
async def leave(ctx, **_):
    ...
    try:
        ...
        raise util.BattleBananaException(ctx.channel, "Could not leave server!")
...
async def optout(ctx, **details):
    ...
    if player.is_playing():
        ...
        if await optout_is_topdog_check(ctx.channel, player):
            return
        if current_permission >= Permission.MOD:
...
async def exchange(ctx, amount, currency, **details):
    ...
    if currency == "DUC":
        raise util.BattleBananaException(ctx.channel, "There is no reason to exchange DUC for DUC!")
    ...
    if amount > discoin.MAX_TRANSACTION:
        raise util.BattleBananaException(ctx.channel, "The amount you try to exchange exceeds the maximum DUC transfer limit of %s." % discoin.MAX_TRANSACTION)
    ...
    if response.get('statusCode'):
        try:
            raise util.BattleBananaException(ctx.channel, "Something went wrong at Discoin! %s, %s" % (response['statusCode'], response['error']))
        except:
            raise util.BattleBananaException(ctx.channel, "Something went wrong at Discoin! " + str(response))
...
@commands.command(args_pattern="S?", permission=Permission.ADMIN)
async def status(ctx, message=None, **details):
    ...
    if message is None:
        ...
        for shard in util.shard_clients:
            ...
            game = discord.Game(name="battlebanana.xyz | shard %d/%d" % (shardID, count))
