...
import battlebanana.game.awards as game_awards
...
async def prestige(ctx, cnf="", **details):
    ...
    if user.money < req_money:
        raise util.BattleBananaException(ctx.channel, "You need atleast %s %s to afford the next prestige!" % (util.format_number_precise(req_money), e.DUT))
