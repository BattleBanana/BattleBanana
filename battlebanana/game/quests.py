        ...
        if message is not None:
            if message.server in quest_map:
                if name.lower() in quest_map[message.server]:
                    raise util.BattleBananaException(message.channel, "A foe with that name already exists on this server!")
...
class ActiveQuest(Player, util.SlotPickleMixin):
    ...
    async def _calculate_stats(self):
        try:
            self.quester.prestige_level = self.quester.prestige_level * 1
        except AttributeError:
...
def _load():
    def load_default_quests():
        with open('battlebanana/game/configs/defaultquests.json') as defaults_file:
