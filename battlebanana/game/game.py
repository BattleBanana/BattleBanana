...
def getResponses():
    return json.load(open("battlebanana/game/configs/daily.json", "r"))
...
async def player_message(message, player, spam_level):
    ...
    if player is not None:
        ...
        if progress_time(player) and spam_level < SPAM_TOLERANCE:
            ...
            # BattleBanana tech award
            if dbconn.conn()["battlebananatechusers"].find({"_id": player.id}).count() > 0:
                if "BattleBananaTech" not in player.awards:
                    player.inventory["themes"].append("battlebanana.tech")
                await awards.give_award(message.channel, player, "BattleBananaTech", "<https://battlebanana.xyz/>")

            ### BattleBanana - the hidden spelling game!
