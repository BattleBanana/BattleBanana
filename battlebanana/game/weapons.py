    ...
    def __init__(self, name, hit_message, damage, accy, **extras):
        ...
        if message is not None:
            if does_weapon_exist(message.server.id, name):
                raise util.BattleBananaException(message.channel, "A weapon with that name already exists on this server!")
...
def _load():
    def load_stock_weapons():
        with open('battlebanana/game/configs/defaultweapons.json') as defaults_file:
