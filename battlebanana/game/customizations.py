...
class Banner(Customization):
    ...
    This class is based off a legacy class from BattleBanana V1
    ...
    def banner_restricted(self, player):
        ...
        return ((not self.admin_only or self.admin_only
                 and permissions.has_permission(member, Permission.ADMIN))
                and (not self.mod_only or self.mod_only
                     and permissions.has_permission(member, Permission.MOD)))
...
class _Teams(dict):
    ...
    def _load_teams(self):
        self.clear()
        with open('battlebanana/game/configs/teams.json') as teams_file:
